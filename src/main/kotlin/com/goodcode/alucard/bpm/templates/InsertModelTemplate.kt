...
@Service
class InsertModelTemplate(
    journeyGateway: JourneyGateway,
    private val kafkaTemplate: KafkaTemplate<String, Any>,
    @Value("\${bpm.processDefinitions.insertModel}") private val insertModel: String,
    @Value("\${kafka.topics.fetchTasks}") private val fetchTasksTopic: String,
) : BpmActionTemplate(journeyGateway) {

    @KafkaListener(topics = ["\${kafka.topics.startInsertModelInstance}"], groupId = "\${kafka.group-id}")
    override fun start(bpmInstanceRequest: BpmInstanceRequest) {
        startBpmInstance(insertModel, bpmInstanceRequest)
